...
namespace camera_aravis {
    ...
    CameraAravisNodelet::~CameraAravisNodelet() {
        for (int i = 0; i < p_streams_.size(); i++) {
            if (p_streams_[i]) { arv_stream_set_emit_signals(p_streams_[i], FALSE); }
        }
        ...
        if (software_trigger_thread_.joinable()) { software_trigger_thread_.join(); }

        tf_thread_active_ = false;
        if (tf_dyn_thread_.joinable()) { tf_dyn_thread_.join(); }
        ...
        if (p_device_) { aravis::device::execute_command(p_device_, "AcquisitionStop"); }

        for (int i = 0; i < p_streams_.size(); i++) { g_object_unref(p_streams_[i]); }
    ...
    void CameraAravisNodelet::onInit() {
        ...
        pub_ext_camera_info_ =
            pnh.param<bool>("ExtendedCameraInfo", pub_ext_camera_info_);  // publish an extended camera info message
        pub_tf_optical_ =
            pnh.param<bool>("publish_tf", pub_tf_optical_);  // should we publish tf transforms to camera optical frame?
        ...
        aravis::camera::bounds::get_frame_rate(p_camera_, &config_min_.AcquisitionFrameRate,
        ...
        for (int i = 0; i < num_streams_; i++) {
            ...
            if (implemented_features_["Gain"]) { aravis::camera::set_gain(p_camera_, config_.Gain); }
        ...
        config_.AcquisitionMode = implemented_features_["AcquisitionMode"]
                                      ? aravis::device::feature::get_string(p_device_, "AcquisitionMode")
                                      : "Continuous";
        config_.AcquisitionFrameRate =
            implemented_features_["AcquisitionFrameRate"] ? aravis::camera::get_frame_rate(p_camera_) : 0.0;
        config_.ExposureAuto = implemented_features_["ExposureAuto"]
                                   ? aravis::device::feature::get_string(p_device_, "ExposureAuto")
                                   : "Off";
        config_.ExposureTime =
            implemented_features_["ExposureTime"] ? aravis::camera::get_exposure_time(p_camera_) : 0.0;
        config_.GainAuto =
        ...
        config_.TriggerMode = implemented_features_["TriggerMode"]
                                  ? aravis::device::feature::get_string(p_device_, "TriggerMode")
                                  : "Off";
        config_.TriggerSource = implemented_features_["TriggerSource"]
                                    ? aravis::device::feature::get_string(p_device_, "TriggerSource")
                                    : "Software";
        ...
        if (pub_tf_optical_) {
            ...
            if (tf_publish_rate > 0.) {
                // publish dynamic tf at given rate (recommended when running as a Nodelet, since latching has
                // bugs-by-design)
        ...
        if (calib_urls[0].empty()) {
            ...
            GuardedGError error;
            bool is_implemented =
                arv_gc_feature_node_is_implemented(ARV_GC_FEATURE_NODE(p_gc_node), error.storeError());
            LOG_GERROR_ARAVIS(error);

            if (is_implemented) {
                // If the feature DeviceSerialNumber is not string, it indicates that the camera is using an older
                // version of the genicam SFNC. Older camera models do not have a DeviceSerialNumber as string, but as
                // integer and often set to 0. In those cases use the outdated DeviceID (deprecated since genicam SFNC
                // v2.0).
        ...
        for (int i = 0; i < num_streams_; i++) {
            ...
            if (!stream_names_[i].empty()) camera_info_frame_id = config_.frame_id + '/' + stream_names_[i];
            ...
            if (!stream_names_[i].empty()) {
                ...
                p_camera_info_managers_[i].reset(new camera_info_manager::CameraInfoManager(
                    *p_camera_info_node_handles_[i], camera_info_frame_id, calib_urls[i]));
            } else {
                p_camera_info_managers_[i].reset(
                    new camera_info_manager::CameraInfoManager(pnh, camera_info_frame_id, calib_urls[i]));
            }
        ...
        const char* device_type = aravis::device::is_uv(p_device_)
                                      ? "USB3Vision"
                                      : (aravis::device::is_gv(p_device_) ? "GigEVision" : "Other");
        ...
        ROS_INFO("    Acquisition Mode     = %s",
                 implemented_features_["AcquisitionMode"]
                     ? aravis::device::feature::get_string(p_device_, "AcquisitionMode")
                     : "(not implemented in camera)");
        ROS_INFO("    Trigger Mode         = %s", implemented_features_["TriggerMode"]
                                                      ? aravis::device::feature::get_string(p_device_, "TriggerMode")
                                                      : "(not implemented in camera)");
        ROS_INFO("    Trigger Source       = %s", implemented_features_["TriggerSource"]
                                                      ? aravis::device::feature::get_string(p_device_, "TriggerSource")
                                                      : "(not implemented in camera)");
        ...
        if (implemented_features_["ExposureTime"]) {
            ...
            ROS_INFO("    Exposure             = %g us in range [%g,%g]", config_.ExposureTime,
                     config_min_.ExposureTime, config_max_.ExposureTime);
        }
        ...
        if (implemented_features_["Gain"]) {
            ...
            ROS_INFO("    Gain                 = %f %% in range [%f,%f]", config_.Gain, config_min_.Gain,
                     config_max_.Gain);
        }
        ...
        if (implemented_features_["GevSCPSPacketSize"])
            ROS_INFO("    Network mtu          = %lu",
                     aravis::device::feature::get_integer(p_device_, "GevSCPSPacketSize"));
    ...
    void CameraAravisNodelet::spawnStream() {
        ...
        for (int i = 0; i < num_streams_; i++) {
            ...
            cam_pubs_[i] = p_transport->advertiseCamera(ros::names::remap(topic_name + "/image_raw"), 1, image_cb,
                                                        image_cb, info_cb, info_cb);
        }
        ...
        for (int i = 0; i < num_streams_; i++) {
            ...
            data->can = this;
            data->stream_id = i;
            g_signal_connect(p_streams_[i], "new-buffer", (GCallback) CameraAravisNodelet::newBufferReadyCallback,
                             data);
        }
        g_signal_connect(p_device_, "control-lost", (GCallback) CameraAravisNodelet::controlLostCallback, this);

        for (int i = 0; i < num_streams_; i++) { arv_stream_set_emit_signals(p_streams_[i], TRUE); }
    ...
    void CameraAravisNodelet::cameraAutoInfoCallback(const CameraAutoInfoConstPtr& msg_ptr) {
        if (config_.AutoSlave && p_device_) {
            ...
            if (strcmp("The Imaging Source Europe GmbH", aravis::camera::get_vendor_name(p_camera_)) == 0) {
                aravis::device::feature::set_integer(p_device_, "WhiteBalanceRedRegister",
                                                     (int) (auto_params_.wb_red * 255.));
                aravis::device::feature::set_integer(p_device_, "WhiteBalanceGreenRegister",
                                                     (int) (auto_params_.wb_green * 255.));
                aravis::device::feature::set_integer(p_device_, "WhiteBalanceBlueRegister",
                                                     (int) (auto_params_.wb_blue * 255.));
    ...
    void CameraAravisNodelet::syncAutoParameters() {
        ...
        if (p_device_) {
            ...
            if (strcmp("The Imaging Source Europe GmbH", aravis::camera::get_vendor_name(p_camera_)) == 0) {
                ...
                auto_params_.wb_green =
                    aravis::device::feature::get_integer(p_device_, "WhiteBalanceGreenRegister") / 255.;
                auto_params_.wb_blue =
                    aravis::device::feature::get_integer(p_device_, "WhiteBalanceBlueRegister") / 255.;
            }
            // the standard way
            else if (implemented_features_["BalanceRatio"] && implemented_features_["BalanceRatioSelector"]) {
                aravis::device::feature::set_string(p_device_, "BalanceRatioSelector", "Red");
    ...
    void CameraAravisNodelet::setExtendedCameraInfo(std::string channel_name, size_t stream_id) {
        if (pub_ext_camera_info_) {
            if (channel_name.empty()) {
                extended_camera_info_pubs_[stream_id] =
                    getNodeHandle().advertise<ExtendedCameraInfo>(ros::names::remap("extended_camera_info"), 1, true);
            } else {
                extended_camera_info_pubs_[stream_id] = getNodeHandle().advertise<ExtendedCameraInfo>(
                    ros::names::remap(channel_name + "/extended_camera_info"), 1, true);
            }
        } else {
    ...
    void CameraAravisNodelet::rosReconfigureCallback(Config& config, uint32_t level) {
        ...
        if (config.frame_id == "") config.frame_id = this->getName();

        // Limit params to legal values.
        config.AcquisitionFrameRate =
            CLAMP(config.AcquisitionFrameRate, config_min_.AcquisitionFrameRate, config_max_.AcquisitionFrameRate);
        ...
        if (config.TriggerMode.compare("Off") != 0) {
            ...
            ROS_WARN("TriggerMode is active (Trigger Source: %s). Cannot manually set AcquisitionFrameRate.",
        ...
        if (changed_auto_master) { setAutoMaster(config.AutoMaster); }

        if (changed_auto_slave) { setAutoSlave(config.AutoSlave); }
    ...
    void CameraAravisNodelet::rosConnectCallback() {
        if (p_device_) {
            if (std::all_of(cam_pubs_.begin(), cam_pubs_.end(),
                            [](image_transport::CameraPublisher pub) { return pub.getNumSubscribers() == 0; })) {
                aravis::device::execute_command(p_device_,
                                                "AcquisitionStop");  // don't waste CPU if nobody is listening!
            } else {
                aravis::device::execute_command(p_device_, "AcquisitionStart");
            }
        }
    ...
    void CameraAravisNodelet::newBufferReady(ArvStream* p_stream,
        ...
        if (n_available_buffers == 0) { p_can->p_buffer_pools_[stream_id]->allocateBuffers(1); }
        ...
        if (p_buffer != NULL) {
            if (arv_buffer_get_status(p_buffer) == ARV_BUFFER_STATUS_SUCCESS && p_can->p_buffer_pools_[stream_id] &&
                ...
                if (p_can->pub_ext_camera_info_) {
                    ...
                    if (arv_camera_is_gv_device(p_can->p_camera_))
                        aravis::camera::gv::select_stream_channel(p_can->p_camera_, stream_id);
                ...
                if (p_can->use_ptp_stamp_) internal::resetPtpClock(p_can->p_device_);
            } else {
                if (arv_buffer_get_status(p_buffer) != ARV_BUFFER_STATUS_SUCCESS) {
                    ROS_WARN("(%s) Frame error: %s", frame_id.c_str(),
                             szBufferStatusFromInt[arv_buffer_get_status(p_buffer)]);
                }
    ...
    void CameraAravisNodelet::writeCameraFeatureFromRosparam(const XmlRpc::XmlRpcValue::iterator::value_type& iter) {
        ...
        if (p_gc_node && arv_gc_feature_node_is_implemented(ARV_GC_FEATURE_NODE(p_gc_node), &error)) {
            ...
            //				ROS_INFO("%s cameratype=%lu, rosparamtype=%d", key.c_str(), typeValue,
            //static_cast<int>(iter.second.getType()));

            // We'd like to check the value types too, but typeValue is often given as G_TYPE_INVALID, so ignore it.
            switch (iter.second.getType()) {
                case XmlRpc::XmlRpcValue::TypeBoolean:  // if
                ...
                case XmlRpc::XmlRpcValue::TypeInt:  // if ((iter.second.getType()==XmlRpc::XmlRpcValue::TypeInt))// &&
                                                    // (typeValue==G_TYPE_INT64))
                    {
                ...
                case XmlRpc::XmlRpcValue::TypeDouble:  // if
                ...
                case XmlRpc::XmlRpcValue::TypeString:  // if
