...
namespace camera_aravis {
    void CameraAravisNodelet::print_capabilities() {
        ...
        bool frame_rate_available = aravis::camera::is_frame_rate_available(p_camera_);
        ROS_INFO("    Can set FrameRate:     %s", btoA(frame_rate_available));
        if (implemented_features_["AcquisitionFrameRate"]) {
            ROS_INFO("    AcquisitionFrameRate = %g hz", aravis::camera::get_frame_rate(p_camera_));
        }


        bool exposure_available = aravis::camera::is_exposure_time_available(p_camera_);
        ...
        if (exposure_available) {
            ...
            double min, max, val = aravis::camera::get_exposure_time(p_camera_);
            aravis::camera::bounds::get_exposure_time(p_camera_, &min, &max);
            ROS_INFO("    Exposure             = %g us in range [%g,%g]", val, min, max);
        }


        bool gain_available = aravis::camera::is_gain_available(p_camera_);
        ROS_INFO("    Can set Gain:          %s", btoA(gain_available));
        if (gain_available) {
            ROS_INFO("    Can set GainAuto:      %s", btoA(implemented_features_["GainAuto"]));

            double min, max, val = aravis::camera::get_gain(p_camera_);
            aravis::camera::bounds::get_gain(p_camera_, &min, &max);
